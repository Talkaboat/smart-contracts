...
abstract contract Liquify is ERC20, ReentrancyGuard, Ownable, TimeLock {
    ...
    IKaiDexRouter public _router;
    address public _factory = 0xC9567a8B6b622cdc8076C6b4432Ade0e11F50Da1;
    ...
    function setLiquidityPair(address _tokenB) public onlyMaintainerOrOwner locked("lp_pair") {
        ...
        _liquidityPair = IUniswapV2Factory(_factory).getPair(address(this), _tokenB);
        if(_liquidityPair == address(0)) {
            _liquidityPair = IUniswapV2Factory(_factory).createPair(address(this), _tokenB);
        }
        excludeTransferFeeAsSender(address(_liquidityPair));
        emit ChangedLiquidityPair(msg.sender, _liquidityPair);
    }

    function setLPAddress(address token) public onlyMaintainerOrOwner locked("lp_pair_address") {
        require(token != address(0), "Liquify::setLiquidityPair: Liquidity pair can't contain zero address");
        _liquidityPair = token;
        excludeTransferFeeAsSender(address(_liquidityPair));
        emit ChangedLiquidityPair(msg.sender, _liquidityPair);
    }
    ...
    function disableAutomaticLiquify() public onlyMaintainerOrOwner {
        isLiquifyActive = true;
    }

    function enableAutomaticLiquify() public onlyMaintainerOrOwner {
    ...
    function updateRouter(address router) public onlyMaintainerOrOwner locked("router") {
        _router = IKaiDexRouter(router);
        setLiquidityPair(_weth);
    ...
    */
    function swapAndLiquify() public taxFree {
        ...
        if (contractTokenBalance >= _minAmountToLiquify) {
            ...
            bool isWeth = tokenA == _weth || tokenB == _weth;
    ...
    function addLiquidityETH(uint256 tokenAmount, uint256 ethAmount) internal {
        ...
        // add the liquidity
        _router.addLiquidityKAI{value: ethAmount}(
            address(this),
            tokenAmount,
