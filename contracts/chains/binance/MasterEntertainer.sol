...
contract MasterEntertainer is Ownable, ReentrancyGuard, PriceTicker, IMasterEntertainer {
    ...
    function setLockPeriodState(bool state) public onlyMaintainerOrOwner {
        isLockPeriodActive = state;
    }
    ...
    function canClaimRewards(uint256 _amount) public view returns (bool) {
        return IERC20(address(coin)).balanceOf(address(this)).sub(depositedCoins) >= _amount;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(canClaimRewards(coinReward + coinReward.div(10))) {
            safeCoinTransfer(devAddress, coinReward.div(10));
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(pending > 0 && canClaimRewards(pending)) {
            safeCoinTransfer(msg.sender, pending);
        }
        if(_amount > 0) {
            require(!isLockPeriodActive || user.lastDeposit.add(pool.lockPeriod * 1 days) <= block.timestamp, "ABOAT::withdraw: Can't withdraw before locking period ended.");
    ...
    function claim(uint256 _pid) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCoinPerShare).div(1e12).sub(user.rewardDebt);
        require(canClaimRewards(pending), "ABOAT::claim: Not enough ABOAT in contract for claim!");
        if (pending > 0) {
            safeCoinTransfer(msg.sender, pending);
    ...
    function withdrawWithoutRewards(uint256 _pid) public nonReentrant {
        PoolInfo storage pool = poolInfos[_pid];
        UserInfo storage user = userInfos[_pid][msg.sender];
        require(!isLockPeriodActive || user.lastDeposit.add(pool.lockPeriod * 1 days) <= block.timestamp, "ABOAT::withdrawWithoutRewards: Can't withdraw before locking period ended.");
